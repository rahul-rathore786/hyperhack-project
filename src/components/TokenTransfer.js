...
const TokenTransfer = ({ coinContract }) => {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (hasMint) {
        ...
        tx = await coinContract.mint(
          recipientAddress,
          ethers.utils.parseEther(amount)
        );
      } else {
        // If you're transferring from your balance
        tx = await coinContract.transfer(
          recipientAddress,
          ethers.utils.parseEther(amount)
        );
      }
      ...
      setMessage({
        text: `Successfully sent ${amount} COIN to ${recipientAddress.substring(
          0,
          6
